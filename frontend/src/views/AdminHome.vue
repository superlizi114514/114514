<template>
  <div class="admin-home-page">
    ...
    <div class="auth-card" v-if="!isAuthenticated">
      ...
      <h2>管理员验证</h2>
      <p class="auth-tip">仅限 admin@admin 用户访问</p>
          ...
          <div v-for="user in userList" :key="user.id" class="user-item">
            <div class="user-info">
              ...
              <div class="tags">
                ...
                <van-tag v-else-if="user.email === 'admin@admin'" type="danger" size="small">管理员</van-tag>
              </div>
              <div class="user-meta">
            ...
            <div class="user-actions-popup">
              ...
              <van-button size="mini" type="success" plain @click="showGrantSupport(user)" v-if="user.email !== 'admin@admin'">赞助</van-button>
              <van-button size="mini" type="warning" plain @click="showSetVip(user)" v-if="user.email !== 'admin@admin'">设置 VIP</van-button>
              <van-button size="mini" type="danger" plain @click="confirmDeleteUser(user)" v-if="user.email !== 'admin@admin'">删除</van-button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="popup-content" style="width: 90%; max-width: 400px; padding: 24px;">
        ...
        <div v-if="currentUser" class="user-detail">
          ...
          <div class="detail-actions">
            <van-button plain block type="primary" @click="resetUserReviews(currentUser)" v-if="currentUser.email !== 'admin@admin'">刷新点评次数</van-button>
            <van-button plain block type="warning" @click="resetUserPassword(currentUser)" v-if="currentUser.email !== 'admin@admin'">重置密码</van-button>
          </div>
        </div>
      </div>
    </van-popup>
